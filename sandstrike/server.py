...
# Ensure `.env` is loaded for the backend process (for local dev).
# `load_env_file()` defaults to cwd; also try repo root so `sandstrike ui start`
# works even if launched from another directory.
try:
    repo_root_env = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".env"))
    load_env_file(".env")
    load_env_file(repo_root_env)
except Exception:
    # Never crash server startup due to env loading
    pass
...
class AvenlisServer:
    ...
    def _setup_routes(self):
        ...
        def run_redteam():
            try:
                # Be tolerant of bad/missing JSON; return 400 instead of bubbling to 500
                ...
                is_file_scan = (storage_type == 'yaml') or (scan_type != 'rapid')
                ...
                # Session persistence:
                # - Rapid scans: local DB (existing behavior)
                # - Full scans: file-only sessions.json (no local DB writes; avoids duplicates)
                session_source = 'file' if is_file_scan else 'local'
                if is_file_scan:
                    session_id = (scan_id or "").strip() or f"scan_{int(datetime.now().timestamp())}"
                else:
                ...
                try:
                    for i, prompt_data in enumerate(prompts_to_test):
                        ...
                        # Apply dynamic variable substitution (e.g. {data.user_credentials})
                        try:
                            from sandstrike.storage.hybrid_storage import HybridStorage
                            prompt_text = HybridStorage().substitute_variables_in_prompt(prompt_text, source='all')
                        except Exception:
                            # If substitution fails, fall back to the raw prompt text
                            pass
                        ...
                        try:
                            ...
                            grader_enabled = grader_config.get('enabled', True)
                            ...
                            if (not reason) and getattr(grading_response.result, 'error', None):
                                reason = grading_response.result.error
                            grader_used = grading_response.metadata.get('provider_used', grader_config.get('grader', 'unknown'))
                            ...
                            # If this is a single-prompt run, return a clearer error instead of 500
                            if total_tests == 1:
                                lowered = msg.lower()
                                if "model" in lowered and "not found" in lowered:
                                    ...
                                    return jsonify({
                                        'error': f"Ollama model not found: '{target_model}'. Run `ollama list` and update your Target model.",
                                        'details': msg
                                    }), 400
                                if "ollama server is not running" in lowered or "connection refused" in lowered:
                                    if not is_file_scan:
                                        self.storage.update_session(session_id, status='failed')
                                    return jsonify({
                                        'error': "Ollama is not reachable. Start it with `ollama serve` and try again.",
                                        'details': msg
                                    }), 503
                        ...
                        if is_file_scan:
                            result_id = f"result_{current_prompt_num}_{int(datetime.now().timestamp())}"
                        else:
                        ...
                        progress_callback({
                            'progress': progress + 5,
                            'message': f'Prompt {current_prompt_num}/{total_tests} completed - {status.upper()}',
                            'completed_prompt': result_payload
                        })
                ...
                if is_file_scan:
                    ...
                    # Keep in-memory `results` list (already populated).
                else:
                ...
                if storage_type == 'yaml':
                    try:
                        ...
                        yaml_session_data = {
                            ...
                            'grading_intent': grader_config.get('grader_intent'),
                            'status': 'completed',
                            # Store as naive local ISO string; the frontend renders in user's locale/timezone.
                            'started_at': start_time.isoformat(),
                            'results': []
                        }
        ...
        def get_sessions():
            try:
                ...
                # The frontend formats dates in the user's local timezone.
                for session in sessions:
        ...
        def get_session(session_id):
            try:
                ...
                # Keep timestamps as ISO strings; frontend formats for user locale/timezone.
        ...
        def add_prompt_to_collection(collection_id):
            ...
            try:
                ...
                logger.info(f"Added prompt {prompt_id} to collection {collection_id}")
                return jsonify({
                    'success': True,
                ...
                logger.error(f"Error adding prompt to collection: {e}")
                return jsonify({'error': str(e)}), 500
        ...
        def remove_prompt_from_collection(collection_id, prompt_id):
            ...
            try:
                ...
                logger.error(f"Error removing prompt from collection: {e}")
                return jsonify({'error': str(e)}), 500
        ...
        def create_prompt():
            ...
            try:
                ...
                logger.info(f"Created prompt with ID {prompt_id} in {storage} storage")
                return jsonify({
                    'success': True, 
                    'prompt_id': prompt_id,
        ...
        def delete_prompt(prompt_id):
            ...
            try:
                ...
                if source == 'file':
                    ...
                    if success:
                        logger.info(f"Deleted file-based prompt {prompt_id} from {source_file}")
                        return jsonify({'success': True})
                    else:
        ...
        def _generate_report_via_api(sessions_data, report_type):
            ...
            try:
                ...
                try:
                    # Import dynamically (code is generated into the temp dir)
                    import importlib
                    html_generator = importlib.import_module("html_generator")
                    HTMLReportGenerator = getattr(html_generator, "HTMLReportGenerator")
