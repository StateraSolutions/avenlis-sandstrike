...
- **Multiple Grading Providers**: Anthropic, Google Gemini, Ollama, and Avenlis Copilot support
...
```python
from sandstrike.grading import GradingEngine, grade_llm_rubric
...
```python
from sandstrike.grading import grade_harmful_content
...
```python
from sandstrike.grading import grade_prompt_injection
...
The grading system uses a YAML configuration file (`sandstrike/data/grading_config.yaml`) to define:
...
### Anthropic (Claude)
- **Model**: Set `ANTHROPIC_MODEL` environment variable (required)
- **API Key**: Set `ANTHROPIC_API_KEY` environment variable
- **Features**: Strong reasoning, good JSON-structured outputs

### Google Gemini
- **Model**: Set `GEMINI_MODEL` environment variable (required)
...
1. **API Key Not Set**
   ...
   Solution: Set ANTHROPIC_API_KEY and ANTHROPIC_MODEL environment variables
   ```
