...
The React UI will automatically open in your browser (default `http://localhost:3000`, but it may choose another free port if 3000 is taken).
...
- **Anthropic Grader**: Use Claude models for response evaluation (requires API key + model)
...
#### Public API Scan
- **API Endpoint**: Public API URL (e.g., `https://api.anthropic.com`)
- **Collections**: Select prompt collections to test
- **Individual Prompts**: Choose specific prompts
- **Grader**: AI-powered response evaluation
- **Scan Mode**: Rapid or Full testing
...
The React frontend runs on `http://localhost:3000` by default (but it may choose another free port) and includes:
...
If you use file storage for team sharing, you may want to commit the YAML outputs so teammates can reuse the same prompts/targets/variables.

This repository **gitignores `sandstrike/data/` by default** to avoid accidentally committing user-generated/runtime data. Seed files remain tracked:

- `sandstrike/data/targets.yaml`
- `sandstrike/data/collections.yaml`
- `sandstrike/data/dynamic_variables.yaml`

To enable versioning of file storage outputs in `sandstrike/data/`, remove the `sandstrike/data/*` rules in `.gitignore`.
...
**Need Help?** Check the CLI_CHEATSHEET.md for command reference or use `sandstrike --help` for CLI assistance. Join our [Telegram group](https://t.me/+09uMofU9Dc8yOTM1) for community support.
